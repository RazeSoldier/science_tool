...
/*声明变量*/
$c = 299792458; //光速
$m0 = $_POST["rmm1"]; //用户输入量"物体的质量"
$v = $_POST["rmv1"]; //用户输入量"物体的速度"
...
}else if(($m0 <= 0) or ($v <= 0)){ //判断2个输入值是否有小于等于0的数值
	require_once "../includes/gravity/error_2.php"; //如果ture，则返回错误信息"error_2.php"
}else{ 
echo "<title>相对论动量计算结果/学园都市</title>";
echo "<b>条件量</b></br>";
echo "物体的质量:$m0 kg</br>";
echo "物体的速度:$v m·s<sup>-1</sup></br>";
echo "<hr/>洛伦兹因子:$lorentz_factor</br><b>计算结果</b>:$result kg·m/s"; //输出结果
}